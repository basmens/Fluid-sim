...
namespace Simulation2D
{
    ...
    {
        public static float DensityKernel(float x, float h)
        {
            if (x > h) return 0;
            double value = (double)h - x;
            value *= value;
            double volume = Math.PI / 6 * Math.Pow(h, 4);
            return (float)(value / volume);
        }

        public static float DensityKernelSlope(float x, float h)
        {
            if (x > h) return 0;
            double value = -2 * ((double)h - x);
            double volume = Math.PI / 6 * Math.Pow(h, 4);
            return (float)(value / volume);
        }
